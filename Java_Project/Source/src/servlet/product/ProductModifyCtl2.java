...
//      String uploadPath = "C:\\_z_im\\_z_im\\NewWorkSpace\\Potato\\WebContent\\upload";
      ...
      // 새로 첨부된 이미지 적용
      int i = 1;
      PaddVO new_pvo = new PaddVO();
      while (files.hasMoreElements()) { // 업로드 된 파일 이름 얻어오기
         String file = (String) files.nextElement();
         String fileName = multi.getFilesystemName(file);
         
         new_pvo.setP_id(p_id);
         
         if (i == 1) {
            if(fileName == null) {
               new_pvo.setPa_img1(multi.getParameter("imgpre1"));
            }else {
               new_pvo.setPa_img1(fileName);
            }
         } else if (i == 2) {
            if(fileName == null) {
               new_pvo.setPa_img2(multi.getParameter("imgpre2"));
            }else {
               new_pvo.setPa_img2(fileName);
            }
         } else if (i == 3) {
            if(fileName == null) {
               new_pvo.setPa_img3(multi.getParameter("imgpre3"));
            }else {
               new_pvo.setPa_img3(fileName);
            }
         }
         i++;
      }
      ...
      int result3 = pservice.updateProductImg(new_pvo);
