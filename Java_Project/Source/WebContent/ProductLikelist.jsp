        ...
        <div class = "jck_content_container">
            ...
            <div class = "jck_content_container_div2">
				...
				<div id="ICR_Product_main">
					...
					<div style="height: 200px;margin-top: 150px;">${noneMsg }<br><br>
					<a href="ProductgetCountCtl.do">찜하러가기</a>
					</div></c:if>
						...
						<c:forEach items="${list }" var="pvo">
							<div id="ICR_Product">
								<div id="ICR_Product1">
									...
									<img style="width: 150px; height: 140px; border-radius: 15px;" src="./upload/${pvo.pa_img3}"></a>
								</div>
								<div id="ICR_Product2" style="text-align: left; padding-left: 20px;padding-top: 20px;">
									<a href="ProductContentCtl.do?p_id=${pvo.p_id}&c_lid=${pvo.c_lid}&m_id=${pvo.m_id}&Likeresult=0">${pvo.p_name}</a><br>
									...
									${pvo.p_value}원
									<img id="ICR_p_like" src="./images/p_like.png">
									<img id="ICR_p_like2" src="./images/p_like2.png">
									${pvo.p_like }<br>
									<input class="ICR_p_id" type="hidden" value="${pvo.p_id}">
								</div>
								<p style="float: right; padding-right: 20px; cursor: pointer;"><a href="ProductDeleteLikeCtl.do?p_id=${pvo.p_id}&m_id=${pvo.m_id}">&times;</a></p>
								</div>
						</c:forEach>
					</c:if>
					<div id="ICR_pageNum" style="margin: 0; display: inline-block;">
						<c:if test="${bCount gt 3 }">
							<a href="ProductSelectLikeCtl.do?pageNum=${prePage}">[이전]</a>
							<c:forEach begin="${startPage }" end="${endPage }" var="i">
								<a href="ProductSelectLikeCtl.do?pageNum=<c:out value="${i }"/>">[<c:out value="${i }"/>]</a>
							</c:forEach>
							<a href="ProductSelectLikeCtl.do?pageNum=${nextPage}">[다음]</a>
						</c:if>
					</div>
					</div>
				</div>
...
Copyright © Potato-Market. All Rights Reserved.</div>
   ...
   $(document).ready(function(){
	...
	<script>
	$("#ICR_p_like").on('click', function (e){
		var p_id;
		if($("#ICR_p_like").is(e.target)){
			var p_id2 = $(e.target).next();
			var p_id3 = p_id2.next();
			p_id = p_id3.val();
			alert(p_id);
		}
		$.ajax({
			type: "POST",
			url: "ProductLikeDeleteCtl.do",
			data: {p_id:p_id},
			success : function(sucdata){
				alert("찜목록에서 삭제되었습니다.");
				var ICR_p_like2 = $(e.target).next();
				$(e.target).css('display','none');
				ICR_p_like2.css('display','inline-block');
			},
			error : function(){
