	...
	$("document").ready(function() {
		$(window).on("scroll", function() {
			if ($(window).scrollTop() > 250) {
				$("#JWJhistorylist").css("top", "70px");
				$("#JWJhistorylist").css("transition-duration", "0.7s");
			} else {
				$("#JWJhistorylist").css("top", "420px");
			}
		});
				...
				<c:if test="${not empty noneMsg }"><div style="height: 200px;">${noneMsg }</div></c:if>
					...
					<div id="ICR_pageNum" style="margin: 0; display: inline-block;">
						<c:if test="${bCount gt 0 }">
							<a href="ProductCategoryCtl.do?c_lid=${c_lid }&pageNum=${prePage}">[이전]</a>
							<c:forEach begin="${startPage }" end="${endPage }" var="i">
								<a href="ProductCategoryCtl.do?c_lid=${c_lid }&pageNum=<c:out value="${i }"/>">[<c:out value="${i }"/>]</a>
							</c:forEach>
							<a href="ProductCategoryCtl.do?c_lid=${c_lid }&pageNum=${nextPage}">[다음]</a>
						</c:if>
					</div>
					</div>
				</div>
